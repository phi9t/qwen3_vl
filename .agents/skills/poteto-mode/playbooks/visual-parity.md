...
4. Verify each component against its baseline via image diff on the matching surface using available tooling. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Repeat per component until the diff is zero.
