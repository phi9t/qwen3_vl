...
**You own this task. Plan, review, verify.** Use subagents for independent investigation or implementation only when available and useful; otherwise run the same passes locally and stay in the lead either way.
...
3. Plan the fix. If it crosses a function boundary, `architect` first. If multi-agent tooling is available and the work can be scoped cleanly, delegate implementation to a focused helper with explicit file scope; otherwise own the patch locally. Review the diff yourself.
...
Investigation runs `how` + `why` as independent passes. Use parallel subagents when available; otherwise run both passes locally and keep their evidence separate until synthesis.
