...
**Start every multi-step task by opening a todolist whose first item is to read the Principles section below in full.** The principles ground every trigger below. In your reply, name each principle that shaped a concrete choice.

The remaining triggers live only here:

- Any uncertain change, architecture decision, or "are we sure?" → the **how** skill.
...
- Any prose surface → the **unslop** skill. Your reply to the user is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **skill-creator** and **writing-skills** skills (Codex skill authoring guidance).
- Before commit → run a local diff/prose cleanup pass with **unslop**.
- Shipping UI / IDE / CLI → use the matching surface tooling and harnesses for that surface when available. For bug fixes, reproduce first on the same surface yourself.
- Broken skill mid-task → fix it in its own PR. Don't block. Don't silently work around it.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "run until X") → keep a decision trail via the **show-me-your-work** skill. Commit it when the stakes need an auditable record; keep it local otherwise.
...
**Use subagents for context-heavy or independent work streams when available.**
Prefer discovery-first delegation: if `tool_search` is available, resolve the right helper skill path first, then dispatch.
When your environment supports parallel calls, use `multi_tool_use.parallel` for independent investigations.
Routed workflow skills (`how`, `why`, `interrogate`, `architect`, `reflect`, etc.) keep their own internal execution model. Do not overwrite that pattern with a fixed wrapper choice.

You own every subagent's work. Review the diff and write your own summary; don't pass through what it said. Interrupt-chained resumes silently drop directives, so refresh the context for each subagent with consolidated scope rather than trusting a "done" summary. A second opinion is the same prompt against a different model; agreement is high-signal.
...
- **Never fabricate a link, citation, or record reference.** Link only artifacts you actually produced or read this session.
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "run until X"). Full steps: `playbooks/autonomous-run.md`.
- **Session pickup.** Resuming or taking over in-flight work from local task artifacts, audit logs, git history, or a pushed branch. Full steps: `playbooks/session-pickup.md`.
