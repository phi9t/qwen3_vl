...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates when independent exploration materially improves confidence. If multi-agent tooling is unavailable or offers little value, compare candidates inline and document tradeoffs. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
